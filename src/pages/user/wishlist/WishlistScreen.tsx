...
import { setHotels, setOpenLogin } from '../../../store/slices/userSlice';
import { Avatar, Box, Card, CardContent, CardMedia, Container, Hidden, IconButton, ImageList, ImageListItem, ImageListItemBar, Rating, Tooltip, Typography } from '@mui/material';
import { WishlistEmpty } from '../../../assets/extraImages';
import { BedOutlined, Favorite, FavoriteBorder, Padding, StarBorder } from '@mui/icons-material';
...
const WishListScreen: React.FC = () => {
  ...
  return (
    <Container>
      <Box paddingTop={4}>
      <Typography variant='h5' fontWeight='bold'>Wishlist</Typography>
      </Box> 
    <ImageList
      gap={12}
      sx={{
    ...
    >
      ...
      <ImageList  sx={{padding:14,
        ...
        >
        <Typography variant='h5' display='flex' paddingTop={0}>Your wishlist is empty!</Typography>
              ...
              <ImageListItem sx={{ height: "100% !important" }}>
                <ImageListItemBar 
                  ...
                  actionIcon={<>
                    ...
                    <IconButton
                     onClick={() => handleWishlist(hotel._id,hotel.hotelName)} // Add your wishlist logic here
    ...
    <Box sx={{ display: "flex", justifyContent: "center", padding: 4 }}>
      {currentHotels.length > hotelsPerPage && (
        <Hidden mdDown>
          <Box>
            {Array.from({ length: Math.ceil(currentHotels.length / hotelsPerPage) }).map((_, index) => (
              <Typography
                key={index + 1}
                variant="button"
                sx={{
