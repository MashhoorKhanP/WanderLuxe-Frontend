...
import { Apartment, ChevronLeft, Dashboard, Discount, Hotel, Inbox, Logout, Mail, PeopleAlt, Redeem, Style, ViewCarousel, Villa } from '@mui/icons-material';
...
const SideList: React.FC<SideListProps> = ({ open, setOpen ,updateTitle }) => {
  ...
  const list = useMemo(() => [
    ...
    {title:'Hotels', icon:<Apartment/>, link:'hotels', component:<Hotels {...{setSelectedLink, link:'hotels'}}/>},
    {title:'Rooms', icon:<Hotel/>, link:'rooms', component:<Rooms {...{setSelectedLink, link:'rooms'}}/>},
    {title:'Bookings', icon:<Style/>, link:'bookings', component:<Bookings {...{setSelectedLink, link:'bookings'}}/>},
    {title:'Coupons', icon:<Redeem/>, link:'coupons', component:<Coupons {...{setSelectedLink, link:'coupons'}}/>},
    {title:'Offers', icon:<Discount/>, link:'offers', component:<Offers {...{setSelectedLink, link:'offers'}}/>},
    {title:'Banners', icon:<ViewCarousel/>, link:'banners', component:<Banners {...{setSelectedLink, link:'banners'}}/>}
        ...
        <Divider />
            ...
            currentUser?.message?.profileImage || currentUser?.profileImage */}
              <Tooltip title={ currentAdmin?.firstName}>
                  <Avatar src={ currentAdmin?.profileImage }
                  {...(open && {sx:{width:100, height:100}})}

                  >{currentAdmin?.firstName?.charAt(0).toUpperCase()}</Avatar>
              </Tooltip>
            </Box>
