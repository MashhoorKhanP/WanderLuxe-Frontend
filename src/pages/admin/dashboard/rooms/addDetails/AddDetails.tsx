...
import { Box, Button, Checkbox, FormControl, FormControlLabel, Grid, InputAdornment, InputLabel, List, ListItem, MenuItem, Popover, Select, Stack, TextField, Tooltip, Typography } from '@mui/material';
...
const [hotelPopoverAnchor, setHotelPopoverAnchor] = useState<HTMLElement | null>(null);
...
const handleViewButtonClick = (event: React.MouseEvent<HTMLElement>,hotelId:string) => {
  setHotelPopoverAnchor(event.currentTarget);
  setImageIndex(0);
      ...
      <Grid item xs={6}>
        ...
        <Grid item xs={6}>
          <TextField
            ...
            onChange={(e) => {
              const selectedHotel = hotels.find((hotel: Hotel) => hotel.hotelName === e.target.value);
              if (selectedHotel) {
          ...
          >
            {hotels.map((hotel:any) => (
              <MenuItem key={hotel._id} value={hotel.hotelName}>
                ...
                <Button variant="outlined" size="small" style={{ position: 'absolute', right: '8%', top: '50%', transform: 'translateY(-50%)' }} endIcon={<RemoveRedEye />} onClick={(e) => handleViewButtonClick(e,hotel._id)}>
                  View
                </Button>
              </Box>
              </MenuItem>
  ...
  <Box p={1}>
    ...
    <Box display="flex" flexDirection={'row'} justifyContent="center" alignItems="center">
      <Tooltip title='Previous'>
        <Button startIcon={<ArrowLeft/>} onClick={() => setImageIndex((prevIndex) => (prevIndex - 1 + hotels[imageIndex].images.length) % hotels[imageIndex].images.length)}></Button>
      </Tooltip>
      <Tooltip title='Next'>
        <Button endIcon={<ArrowRight/>} onClick={() => setImageIndex((prevIndex) => (prevIndex + 1) % hotels[imageIndex].images.length)}></Button>
      </Tooltip>
    </Box>
    <ul style={{ listStyle: 'none', padding: 0 }}>
    <li>
      <Typography variant="h6" sx={{ fontSize: '14px',fontWeight:'normal', p: 1 }}>
